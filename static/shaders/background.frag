...
vec2 hash(float n) {
    vec2 p = fract(vec2(n * 5.398, n * 5.442));
    p += dot(p.yx, p.xy + vec2(25.967, 17.312));
    return fract(vec2(p.x * p.y * 95.433, p.x * p.y * 97.59)) * 2.f - 1.f;
}
...
void main(void) {
    ...
    vec2 uv = texCoord + SHAKE_INTENSITY * u_treble * hash(u_time);

    vec4 grayScale = texture(source, uv);
    grayScale = vec4(vec3(0.2126 * grayScale.r + 0.7152 * grayScale.g + 0.0722 * grayScale.b), 1.f);
    
    float aberrationFactor = ABERRATION_INTENSITY * u_beat;

    vec2 blurDir = vec2(uv - 0.5);
    vec4 colorTex = sampleWithAberration(source, uv, aberrationFactor) * 0.5f;
